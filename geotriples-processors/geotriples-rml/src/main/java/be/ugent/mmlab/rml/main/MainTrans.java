...
public class MainTrans {
	 ...
	 * @throws Exception
	 */
	private final static Logger log = LoggerFactory.getLogger(MainTrans.class);
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			// add options
			options.addOption("variables",true,"variables for use in mapping");
			...
			if(cmd.hasOption("variables")){
				String variables= cmd.getOptionValue("variables");
				String[] vars = variables.split("\\s+");
				for(String var :vars){
					String[] parts = var.split("=");
					if(parts.length==2){
						be.ugent.mmlab.rml.core.Config.variables.put(parts[0].trim(), parts[1].trim());
					}
				}
			}
			...
			LoggerFactory.getLogger(MainTrans.class.getName()).error(ex.toString());
		}
