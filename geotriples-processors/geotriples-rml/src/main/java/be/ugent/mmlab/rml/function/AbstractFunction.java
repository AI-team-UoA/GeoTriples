...
public abstract class AbstractFunction {
	private final static Logger log = LoggerFactory.getLogger(AbstractFunction.class);

	protected Geometry computeGeometry(String value, QLTerm term) throws SAXException, IOException,
 ParserConfigurationException, NoSuchAuthorityCodeException,
			FactoryException, MalformedGeometryException, ParseException {
		...
		case XPATH_CLASS:
			...
			if (Config.useGML3) {
				...
				// StringBuffer("<gml:featureMember
				...
				// StringBuffer("<gml:featureMember
				// xmlns:gml=\"http://www.opengis.net/gml\">");
				sbuffer.append(value.replaceAll("<(?=\\w+)", "<kml:").replace("</", "</kml:"));
				...
				ArrayList<?> feature = (ArrayList<?>) dfc.getAttribute("Feature");
			...
			if (geometry == null) {
				if (log.isDebugEnabled()) {
					try {
						throw new Exception("Could not parse the geometry! Value is: " + value);
					} catch (Exception ee) {
						ee.printStackTrace();
					}
				} else {
					if (Config.useKML_22) {
						...
						log.warn("Could not parse the geometry! Please report this incident to the developers (dimis)");
					}
	...
	protected static CacheFifo<Object, Geometry> cache = new CacheFifo<>(44);
