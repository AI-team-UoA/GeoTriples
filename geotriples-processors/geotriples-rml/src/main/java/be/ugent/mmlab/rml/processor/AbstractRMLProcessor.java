...
public abstract class AbstractRMLProcessor implements RMLProcessor {
	 ...
	 */
	// Logger
	private static Logger log = LoggerFactory.getLogger(R2RMLEngine.class);
				...
				if (performersForFunctionInsideJoinCondition.size() > 0) {
					...
					RMLPerformer performer = new JoinReferenceRMLPerformer(
							processor, subject, predicate, identifier
									.toString().trim());
					processor.execute(dataset, tm, performer, fileName,false);
		...
		case TEMPLATE_VALUED:
			...
			for (String expression : tokens) {
				...
				for (int i = 0; i < replacements.size(); i++) {
					...
					// LoggerFactory.getLogger(AbstractRMLProcessor.class.getName()).log(Level.SEVERE,
					// null, ex);
					// }
	 ...
	 */
	@Override
	public Collection<Statement> processPredicateObjectMap(SesameDataSet dataset,
				...
				for (ReferencingObjectMap referencingObjectMap : referencingObjectMaps) {
					...
					if (joinConditions.isEmpty()
						...
						performer = new JoinRMLPerformer(processor, subject,
								predicate);
						statements.addAll(processor.execute(dataset, parentTriplesMap, performer,
								fileName,false));
					}
					// same Logical Source and no Conditions
						...
						if ((parentTriplesMap.getLogicalSource()
							...
							statements.addAll(performer.perform(node, dataset, parentTriplesMap));
						} else {
							...
							statements.addAll(processor.execute_node(dataset, expression,
					...
					// Conditions
						...
						statements.addAll(oldprocessor1.execute(dataset, trmlast, oldperformer1,
								fileName1,true));
