...
// public class SQLMappingGenerator extends RMLMappingGenerator{
...
// 		} else if (jdbcURL.contains("monetdb")) {
...
// 				}
// 			}
// 			if(typeName.equals("geography_columns") || typeName.equals("spatial_ref_sys") || typeName.equals("geometry_columns")){
// 				continue;
// 			}
...
// 				if(columntype.name().equalsIgnoreCase("GEOMETRY")){
// 					triplesMaps.put(typeName,
...
// 					String query = "SELECT *," + " st_dimension(" + identifier + ") as \\\"dimension\\\", st_coorddim(" + identifier
// 							+ ") as \\\"coordinateDimension\\\", st_coorddim(" + identifier
// 							+ ") as \\\"spatialDimension\\\", CASE WHEN st_issimple(" + identifier
// 							+ ") THEN 'true' ELSE 'false' END as \\\"isSimple\\\", CASE WHEN st_isempty(" + identifier
// 							+ ") THEN 'true' ELSE 'false' END as \\\"isEmpty\\\", CASE WHEN st_coorddim("+identifier+")=3 THEN 'true' ELSE 'false' END as \\\"is3D\\\", CONCAT(\'<http://www.opengis.net/def/crs/EPSG/0/\', ST_SRID(" + identifier + "), \'> \' ,st_astext(" + identifier + ")) as \\\"asWKT\\\" FROM " + typeName;
// 					if(!jdbcDriver.equals("org.postgresql.Driver")){
// 					query="SELECT *, st_dimension("+identifier+") as \\\"dimension\\\", st_dimension("+identifier+") as \\\"coordinateDimension\\\", st_dimension("+identifier+") as \\\"spatialDimension\\\",  st_issimple("+identifier+") as \\\"isSimple\\\", st_isempty("+identifier+") as \\\"isEmpty\\\", CASE WHEN st_dimension("+identifier+")=3 THEN 'true' ELSE 'false' END as \\\"is3D\\\", CONCAT('<http://www.opengis.net/def/crs/EPSG/0/4326> ' , REPLACE(CAST("+identifier+" AS TEXT), '\\\"', '')) as \\\"asWKT\\\" FROM "
// 							+ typeName + ";";
// 					}
// 					triplesMaps.put(typeNameGeo,
...
// 									query));
// 					triplesMaps.put(typeNameGeo,
// 							triplesMaps.get(typeNameGeo) + printSubjectMap(baseURI, "Geometry", "ogc", true, primarykey));
