...
public class R2RMLCompiler implements CompiledMapping {
	public final static Logger log = LoggerFactory.getLogger(R2RMLCompiler.class);
	...
	private final Collection<TripleRelation> currentTripleRelations = 
		new ArrayList<TripleRelation>();
	private AliasOp currentTable = null;
	...
	private void visitComponent(TriplesMap triplesMap) {
		...
		NamedOp namedOp = createTabular(triplesMap);
		String schema = null;
		if (namedOp.getTableName().getSchema() != null) {
			schema = namedOp.getTableName().getSchema().getCanonicalName();
		}
		String name = namedOp.getTableName().getTable().getCanonicalName();
		currentTable = AliasOp.create(namedOp, "CHILD"+schema+name);
	...
	private void visitComponent(PredicateObjectMap poMap) {
		...
		//System.out.println(predicateMaps.size());
