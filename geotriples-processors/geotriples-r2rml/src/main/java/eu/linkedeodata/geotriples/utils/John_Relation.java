...
class John_Relation<T> {
    ...
    synchronized public John_Relation<T> copy() {
        John_Relation<T> rslt = new John_Relation<T>();
        Iterator<John_PairEntry<T, T>> it = iterator();
        while ( it.hasNext() ) {
    ...
    // ------------
    private static <X> Map1<X, John_PairEntry<X, X>> inner(final X a)
    {
        return new Map1<X, John_PairEntry<X, X>>() {
            @Override
            public John_PairEntry<X, X> map1(X b)
            {
                return new John_PairEntry<X, X>(a, b) ;
            }
        } ;
    }

    private static <T> Iterator<John_PairEntry<T, T>> pairEntry(Map.Entry<T, Set<T>> pair)
    {
        final T a = pair.getKey() ;
        Set<T> bs = pair.getValue() ;
        return new Map1Iterator<T, John_PairEntry<T, T>>(inner(a), bs.iterator()) ;
    }
     ...
     */   
    public Iterator<John_PairEntry<T, T>> iterator()
    {
        ...
        Map1<Map.Entry<T, Set<T>>, Iterator<John_PairEntry<T, T>>> m1 = 
        new Map1<Map.Entry<T, Set<T>>, Iterator<John_PairEntry<T, T>>>(){
            @Override
            public Iterator<John_PairEntry<T, T>> map1(Entry<T, Set<T>> entry)
            {
                return pairEntry(entry) ;
            }} ;
        
        Map1Iterator<Map.Entry<T, Set<T>>,Iterator<John_PairEntry<T, T>>> iter1 =
            new Map1Iterator<Map.Entry<T, Set<T>>,Iterator<John_PairEntry<T, T>>>(m1 , rows.entrySet().iterator()) ;
        // And now flatten it.
        Iterator<John_PairEntry<T, T>> iter2 = new IteratorIterator<John_PairEntry<T, T>>(iter1) ; 
        return iter2 ;
    }
}
