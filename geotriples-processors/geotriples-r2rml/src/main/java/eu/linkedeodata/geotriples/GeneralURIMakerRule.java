 ...
 *   calls to {@link GeneralURIMakerRuleChecker#canMatch(NodeMaker)} will return false
 ...
 */
public class GeneralURIMakerRule implements Comparator<GeneralTripleRelation> {
	private Map<NodeMaker, URIMakerIdentifier> identifierCache = new HashMap<NodeMaker, URIMakerIdentifier>();

	public List<GeneralTripleRelation> sortRDFRelations(Collection<GeneralTripleRelation> tripleRelations) {
		ArrayList<GeneralTripleRelation> results = new ArrayList<GeneralTripleRelation>(tripleRelations);
		Collections.sort(results, this);
		return results;
	}

	public GeneralURIMakerRuleChecker createRuleChecker(Node node) {
		return new GeneralURIMakerRuleChecker(node);
	}
	
	public int compare(GeneralTripleRelation o1, GeneralTripleRelation o2) {
	...
	public class GeneralURIMakerRuleChecker {
		private final Node node;
		private boolean canMatchURIColumn = true;
		public GeneralURIMakerRuleChecker(Node node) {
			this.node = node;
		}
