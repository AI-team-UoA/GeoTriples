...
import eu.linkedeodata.geotriples.writers.WP2AbbreviatedWriter;
...
public class John_Unparser {
     ...
     *            The output.
     */
    public John_Unparser(WP2AbbreviatedWriter parent, String localName, Model m, PrintWriter w) {
...
//            try 
        {
                IRI u = John_BaseXMLWriter.factory.create(uri);
                u = u.create("");
...
//            }
     ...
     * 
     */
    public void setTopLevelTypes(Resource types[]) {
        pleasingTypes = types;
    ...
    public void setXMLBase(String b) {
        xmlBase = b;
    }
    ...
    final private WP2AbbreviatedWriter prettyWriter;
    ...
    private boolean wPropertyEltLiteral(WType wt, Property prop, Statement s,
            RDFNode r) {
        if (prettyWriter.issParseTypeLiteralPropertyElt())
            return false;
    ...
    private boolean wPropertyEltResource(WType wt, Property prop, Statement s,
            RDFNode r) {
        if (prettyWriter.issParseTypeResourcePropertyElt())
            return false;
    ...
    private boolean wPropertyEltValueObj(WType wt, Property prop, Statement s,
            RDFNode r) {
        if (r instanceof Resource && !prettyWriter.issResourcePropertyElt()) {
            Resource res = (Resource) r;
            done(s);
            tab();
            print("<");
    ...
    private boolean isGenuineAnon(Resource r) {
        ...
        return v == null
                || ((!prettyWriter.issResourcePropertyElt()) && v.intValue() <= 1 && (!haveReified
                        .contains(r)));
    }
...
//            return;
    ...
    private boolean canBeAttribute(Statement s, Set<Property> seen) {
        Property p = s.getPredicate();
        // Check seen first.
        if (prettyWriter.issPropertyAttr() || seen.contains(p)) // We can't use the
                                                            // same attribute
            // twice in one rule.
            return false;
        seen.add(p);
    ...
    private Statement[][] getDamlList(RDFNode r) {
        return prettyWriter.issDamlCollection() ? null : getList(r, DAML_OIL.List,
                DAML_OIL.first, DAML_OIL.rest, DAML_OIL.nil);
    }

    private Statement[][] getRDFList(RDFNode r) {
        return prettyWriter.issParseTypeCollectionPropertyElt() ? null : getList(r,
    ...
    private void findInfiniteCycles() {
        ...
        John_Relation<Resource> relation = new John_Relation<Resource>();
        try {
            while (ss.hasNext()) {
                Statement s = ss.nextStatement();
                if (!doneSet.contains(s)) {
