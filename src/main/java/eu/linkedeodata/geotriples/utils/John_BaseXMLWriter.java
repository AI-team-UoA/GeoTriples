...
/**
 * This is not part of the public API. Base class for XML serializers. All
 * methods with side-effects should be synchronized in this class and its
...
abstract public class John_BaseXMLWriter implements RDFXMLWriterI {

	private static final String newline = JenaRuntime
	...
	public John_BaseXMLWriter() {
		setupMaps();
	}

	private static Logger xlogger = LoggerFactory
			.getLogger(John_BaseXMLWriter.class);
	...
	abstract protected void writeBody(Model mdl, PrintWriter pw,
			String baseUri, boolean inclXMLBase);
	...
	protected RDFErrorHandler errorHandler = defaultErrorHandler;
	...
	protected String anonId(Resource r) {
		return longId ? longAnonId(r) : shortAnonId(r);
	}

	/*
	 * A shortAnonId is computed by maintaining a mapping table from the
	 * internal id's of anon resources. The short id is the index into the table
	 * of the internal id.
	 */
	...
	/**
	 * true means all namespaces defined in the model prefixes will be noted in
	 * xmlns declarations; false means only "required" ones will be noted. Hook
	 * for configuration.
	 */
	private boolean writingAllModelPrefixNamespaces = true;

	private John_Relation<String> nameSpaces = new John_Relation<String>();
	...
	protected void workOutNamespaces() {
		if (ns == null) {
	...
	private void setFromWriterSystemProperties(Map<String, String> ns,
			Set<String> prefixesUsed) {
		Iterator<String> it = namespacesNeeded.iterator();
		while (it.hasNext()) {
			String uri = it.next();
			String val = JenaRuntime
					.getSystemProperty(RDFWriter.NSPREFIXPROPBASE + uri);
			if (val != null && checkLegalPrefix(val)
					&& !prefixesUsed.contains(val)) {
				ns.put(uri, val);
	...
	private void setFromGivenNamespaces(Map<String, String> ns,
			Set<String> prefixesUsed) {
		Iterator<String> it = namespacesNeeded.iterator();
		while (it.hasNext()) {
			String uri = it.next();
	...
	protected String rdfEl(String local) {
		return tag(RDFNS, local, FAST, true);
	}

	protected String startElementTag(String uri, String local) {
	...
	protected String attributeTag(String uriref) {
		return splitTag(uriref, ATTR);
	}

	protected String attributeTag(String uri, String local) {
		return tag(uri, local, ATTR, false);
	}

	protected String rdfAt(String local) {
		return tag(RDFNS, local, FASTATTR, true);
	}

	protected String endElementTag(String uri, String local) {
	...
	String splitTag(String uriref, int type) {
		...
		return tag(uriref.substring(0, split), uriref.substring(split), type,
				true);
	}
	...
	private String cookUpAttribution(int type, String namespace, String local) {
		...
		case FASTATTR:
		case ATTR:
			return "xmlns:" + prefix + "=" + substitutedAttribute(namespace)
					+ " " + prefix + ":" + local;
		case START:
			return prefix + ":" + local + " xmlns:" + prefix + "="
					+ substitutedAttribute(namespace);
		default:
		case END:
			return prefix + ":" + local;
		case FAST:
	 ...
	 * @param base
	 *            the base URI for relative URI calculations. <code> null</code>
	...
	private void writeXMLBody(Model model, PrintWriter pw, String base) {
		...
		// errors?
		if (xmlBase == null) {
			baseURI = (base == null || base.length() == 0) ? null : factory
					.create(base);
	...
	protected static final Pattern predefinedEntityNames = Pattern
	...
	protected String substitutedAttribute(String s) {
		...
		if (!showDoctypeDeclaration.booleanValue())
			...
			return prefix == null || isPredefinedEntityName(prefix) ? attributeQuoted(substituted)
					: attributeQuoted("&" + strForPrefix(prefix) + ";"
	...
	private void writeXMLDeclaration(Writer out, PrintWriter pw) {
		...
		if (out instanceof OutputStreamWriter) {
			...
			if (!(javaEnc.equals("UTF8") || javaEnc.equals("UTF-16"))) {
				...
				decl = "<?xml version=" + attributeQuoted("1.0") + " encoding="
						+ attributeQuoted(ianaEnc) + "?>";
	 ...
	 * 
	 * @param errHandler
	 *            The new error handler to be used, or null for the default
	 *            handler.
	 * @return the old error handler
	 */
	@Override
	synchronized public RDFErrorHandler setErrorHandler(
	 ...
	 * Return an Object representation of the original value.
	 * 
	 * @see com.hp.hpl.jena.rdf.model.RDFWriter#setProperty(java.lang.String,
	 *      java.lang.Object)
	 */
	@Override
	final synchronized public Object setProperty(String propName,
			Object propValue) {
	...
	private String setAttributeQuoteChar(Object propValue) {
		...
		if ("\"".equals(propValue) || "'".equals(propValue))
			...
			logger.warn("attributeQutpeChar must be either \"\\\"\" or \', not \""
					+ propValue + "\"");
		return oldValue;
	}
	...
	private Integer setWidth(Object propValue) {
		...
		if (propValue instanceof Integer) {
			...
			try {
				...
				logger.warn("Bad value for width: '" + propValue + "' ["
						+ e.getMessage() + "]");
			}
		}
		return oldValue;
	...
	private Integer setTab(Object propValue) {
		...
		if (propValue instanceof Integer) {
			...
			try {
				...
				logger.warn("Bad value for tab: '" + propValue + "' ["
						+ e.getMessage() + "]");
			}
		}
		return result;
	...
	private String setShowXmlDeclaration(Object propValue) {
		String oldValue = showXmlDeclaration == null ? null
				: showXmlDeclaration.toString();
		showXmlDeclaration = getBooleanValue(propValue, null);
		return oldValue;
	}

	/**
	 * Answer the boolean value corresponding to o, which must either be a
	 * Boolean, or a String parsable as a Boolean.
	 */
	...
	protected Resource[] setTypes(Resource x[]) {
	...
	/*
	 * private boolean sameDocument = true; private boolean network = false;
	 * private boolean absolute = true; private boolean relative = true; private
	 * boolean parent = true; private boolean grandparent = false;
	 */
	private int relativeFlags = IRI.SAMEDOCUMENT | IRI.ABSOLUTE | IRI.CHILD
			| IRI.PARENT;

	/**
	 * Answer the form of the URI after relativisation according to the
	 * relativeFlags set by properties. If the flags are 0 or the base URI is
	 * null, answer the original URI. Throw an exception if the URI is "bad" and
	 * we demandGoodURIs.
	 */
	protected String relativize(String uri) {
		return relativeFlags != 0 && baseURI != null ? relativize(baseURI, uri)
				: checkURI(uri);
	}
	...
	/**
	 * Answer the argument URI, but if we demandGoodURIs and it isn't good,
	 * throw a JenaException that encapsulates a MalformedURIException. There
	 * doesn't appear to be a convenient URI.checkGood() kind of method, alas.
	 */
	...
	/**
	 * Answer true iff prefix is a "legal" prefix to use, ie, is empty [for the
	 * default namespace] or an NCName that does not start with "xml" and does
	 * not match the reserved-to-Jena pattern.
	 */
	private boolean checkLegalPrefix(String prefix) {
		...
		if (prefix.toLowerCase().startsWith("xml"))
			...
			logger.warn("Namespace prefix '" + prefix
					+ "' is reserved by Jena.");
		else
			return true;
		return false;
	...
	public static int str2flags(String pv) {
		...
		while (tkn.hasMoreElements()) {
			...
			if (flag.equals("same-document"))
				...
				logger.warn("Incorrect property value for relativeURIs: "
						+ flag);
		}
		return rslt;
	}
