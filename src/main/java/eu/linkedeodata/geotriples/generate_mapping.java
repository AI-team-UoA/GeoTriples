...
public class generate_mapping {
	...
	public void process(String[] args) throws Exception {
		if (args.length >= 1) {
			...
			if (lastToken.endsWith(".shp")) {
				...
				if (isRML) {
					final ArgDecl outfileArg = new ArgDecl(true, "o", "out",
							"outfile");
					final ArgDecl baseIRIArg = new ArgDecl(true, "b", "base",
							"Base IRI");
					final ArgDecl ontologyOutArg = new ArgDecl(true, "ont", "onto",
							"outfile for ontology");
					...
					cmd.add(isRMLarg);
					cmd.process(args);
					new ShapefileMappingGenerator(
							lastToken, cmd.getValue(outfileArg),
							cmd.getValue(baseIRIArg), cmd.getValue(ontologyOutArg)).run();
					
				} else {
					if (this.recipe != null) {
						(new eu.linkedeodata.geotriples.shapefile.generate_mapping(
								this.recipe, this.guiOutput)).process(args,
								this.recipe);
					} else {
						(new eu.linkedeodata.geotriples.shapefile.generate_mapping(
								this.recipe, this.guiOutput)).process(args);
				...
				System.out
						.println("Currently GeoPDF is not implemented within WP2 (soon)");
			} else if (lastToken.endsWith(".xml") || lastToken.endsWith(".gml")
					|| lastToken.endsWith(".kml")) {
				log.info("XML detected for processing");
				final ArgDecl outfileArg = new ArgDecl(true, "o", "out",
						"outfile");
				final ArgDecl xsdifileArg = new ArgDecl(true, "x", "xsd",
						"XSD file");
				final ArgDecl baseIRIArg = new ArgDecl(true, "b", "base",
						"Base IRI");
				final ArgDecl rootElement = new ArgDecl(true, "r", "root",
						"Root element to start");
				final ArgDecl nullTypesArg = new ArgDecl(false, "null",
						"nulltypes",
				...
				final ArgDecl namespacesArg = new ArgDecl(true, "ns",
						"namespaces",
						"used for namespaces in root path (-rp | --rootpath)");
				final ArgDecl onlyNamespace = new ArgDecl(true, "onlyns",
						"onlynamespace",
				...
				new XMLMappingGeneratorTrans(cmd.getValue(xsdifileArg),
						lastToken, cmd.getValue(outfileArg),
						cmd.getValue(baseIRIArg), cmd.getValue(rootElement),
						cmd.getValue(rootPathArg), cmd.getValue(namespacesArg),
						cmd.hasArg(nullTypesArg), cmd.getValue(ontologyOutArg),
	...
	public static void usage() {
		...
		System.err
				.println("    -v              Generate RDFS+OWL vocabulary instead of mapping file");
		System.err.println("    --verbose       Print debug information");
		System.err
				.println("    -b <baseIR> e.g. http://geo.linkedopendata.gr/natura");
		System.err
				.println("	-geov			Use target geospatial vocabulary (GeoSPARQL/stRDF)");
		System.err.println();
		System.exit(1);
	}
